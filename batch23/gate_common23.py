# -*- coding: utf-8 -*-
"""batch23 首单元门禁（通用版）：verify selftest 复跑 + 真实页教学链/通关/写档 + clips 注入
用法: python gate_common22.py <game> <hook> [clips_n]
  game: weather|share|piano  hook: WE|SH|PI"""
...
async def main():
    async with async_playwright() as p:
        ...
        demo_var = {'PF': '__pfDemoR', 'AR': '__arDemoR', 'MM': '__mmDemoR', 'CD2': '__cd2DemoR', 'ST': '__stDemoR', 'BC': '__bcDemoR', 'CI': '__ciDemoR', 'GK': '__gkDemoR', 'SN': '__snDemoR', 'TC': '__tcDemoR', 'MD': '__mdDemoR', 'QZ': '__qzDemoR', 'WE': '__weDemoR', 'SH': '__shDemoR', 'PI': '__piDemoR'}[HOOK]
        demo_r = None
        for _ in range(60):
        ...
        for _ in range(30):
            ...
            if stars is not None:
                break
            await pg.wait_for_timeout(500)
        # 演示返回值按款语义（SPEC-BATCH23）：三款演示完成一题均='right'（weather 选对/share 分对/piano 弹对）
