# -*- coding: utf-8 -*-
"""batch23 全量回归 driver：
①三款独立复验（verify_one_weather + verify_one_share + verify_one_piano）
②语音专项 verify_voice（weather/share/piano）
③家长门+输入加关 E2E（core 级，colormix 抽测）
④两级入口（batch22/index.html 三卡+主入口 ≥120 卡含三卡）
...
run('verify:weather', 'python verify_one_weather.py', BASE, timeout=3000)
run('verify:share', 'python verify_one_share.py', BASE, timeout=3000)
run('verify:piano', 'python verify_one_piano.py', BASE, timeout=3000)
run('voice', 'python verify_voice.py weather share piano', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，colormix 抽测；算术题→input 填 5→确定→limit+5）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_share', JSON.stringify({v:'1.0',game:'share',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'share', 'index.html').replace('\\', '/'))
...
def entries():
    with sync_playwright() as p:
        ...
        ok1 = sum(1 for c in cards if '天气穿衣' in c) >= 1 and sum(1 for c in cards if '分糖果' in c) >= 1 and sum(1 for c in cards if '碰碰琴' in c) >= 1
        pg2 = b.new_page()
        ...
        ok2 = sum(1 for c in main_cards if '天气穿衣' in c) >= 1 and sum(1 for c in main_cards if '分糖果' in c) >= 1 and sum(1 for c in main_cards if '碰碰琴' in c) >= 1 and len(main_cards) >= 120  # r9: hub 09-12 扩容 120 卡，b23 收官快照 69 过时，改下限防跌卡（同 r8 feed ==63→≥120 族）
        # 三款产物页直开冒烟
        n = 0
        for g in ('weather', 'share', 'piano'):
            pg3 = b.new_page()
        ...
        b.close()
        assert ok1 and ok2 and ok3, '入口断言 batch22=%s main=%s smoke=%s' % (ok1, ok2, ok3)
    print('[PASS] entries 两级入口+三产物冒烟（主入口 ≥120 卡）')
