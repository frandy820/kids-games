# -*- coding: utf-8 -*-
"""batch7 全量回归 driver：
①三款 verify_one（verify=1/真实操作通关/双viewport/离线/截图/钩子/零惩罚/sayW）
②语音专项 verify_voice（fishcolor/fruitsplit/hopscotch）
③家长门+输入加关 E2E（core 级，fishcolor 抽测）
...
# ① 三款复验
run('verify:fishcolor', 'python verify_one_fishcolor.py', BASE)
run('verify:fruitsplit', 'python verify_one_fruitsplit.py', BASE)
run('verify:hopscotch', 'python verify_one_hopscotch.py', BASE)

# ② 语音专项
run('voice', 'python verify_voice.py fishcolor fruitsplit hopscotch', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，fishcolor 抽测）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_fishcolor', JSON.stringify({v:'1.0',game:'fishcolor',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'fishcolor', 'index.html').replace('\\', '/'))
