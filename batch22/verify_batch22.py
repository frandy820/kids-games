# -*- coding: utf-8 -*-
"""batch22 全量回归 driver：
①三款独立复验（verify_one_hidden + verify_one_slide + verify_one_colormix）
②语音专项 verify_voice（hidden/slide/colormix）
③家长门+输入加关 E2E（core 级，colormix 抽测）
④两级入口（batch22/index.html 三卡+主入口 66 卡含三卡）
...
run('verify:hidden', 'python verify_one_hidden.py', BASE, timeout=3000)
run('verify:slide', 'python verify_one_slide.py', BASE, timeout=3000)
run('verify:colormix', 'python verify_one_colormix.py', BASE, timeout=3000)
run('voice', 'python verify_voice.py hidden slide colormix', os.path.join(BASE, '..', 'batch1'))
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_colormix', JSON.stringify({v:'1.0',game:'colormix',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'colormix', 'index.html').replace('\\', '/'))
...
def entries():
    with sync_playwright() as p:
        ...
        ok1 = sum(1 for c in cards if '隐藏朋友' in c) >= 1 and sum(1 for c in cards if '滑块拼图' in c) >= 1 and sum(1 for c in cards if '颜色魔法' in c) >= 1
        pg2 = b.new_page()
        ...
        ok2 = sum(1 for c in main_cards if '隐藏朋友' in c) >= 1 and sum(1 for c in main_cards if '滑块拼图' in c) >= 1 and sum(1 for c in main_cards if '颜色魔法' in c) >= 1 and len(main_cards) == 66
        # 三款产物页直开冒烟
        n = 0
        for g in ('hidden', 'slide', 'colormix'):
            pg3 = b.new_page()
    ...
    print('[PASS] entries 两级入口+三产物冒烟（主入口 66 卡）')
