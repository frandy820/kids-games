# -*- coding: utf-8 -*-
"""batch22 首单元门禁（通用版）：verify selftest 复跑 + 真实页教学链/通关/写档 + clips 注入
用法: python gate_common22.py <game> <hook> [clips_n]
  game: hidden|slide|colormix  hook: HD|SL|CM"""
...
async def main():
    async with async_playwright() as p:
        ...
        demo_var = {'PF': '__pfDemoR', 'AR': '__arDemoR', 'MM': '__mmDemoR', 'CD2': '__cd2DemoR', 'ST': '__stDemoR', 'BC': '__bcDemoR', 'CI': '__ciDemoR', 'GK': '__gkDemoR', 'SN': '__snDemoR', 'TC': '__tcDemoR', 'MD': '__mdDemoR', 'QZ': '__qzDemoR', 'HD': '__hdDemoR', 'SL': '__slDemoR', 'CM': '__cmDemoR'}[HOOK]
        demo_r = None
        for _ in range(60):
            ...
            await pg.wait_for_timeout(500)
        tut = 'watch'
        for _ in range(40):                       # 等教学 handoff 完成（watch→help/solo；长教学款 demoR 出现≠交接完）
        ...
        for _ in range(30):
            ...
            if stars is not None:
                break
            await pg.wait_for_timeout(500)
        # 演示返回值按款语义（SPEC-BATCH22）：hidden 演示找到 1 只='found'；slide 演示滑 1 块='slide'；colormix 演示调对='right'
        demo_ok = demo_r == 'right' or (GAME == 'hidden' and demo_r == 'found') or (GAME == 'slide' and demo_r == 'slide')
        ...
        # G3 clips 注入（manifest games 全集）
        ctx = await b.new_context()
        pg = await ctx.new_page()
        await pg.goto(URL_R)
        await pg.wait_for_timeout(1800)
        mani = open(BASE.parent / 'voice' / 'clips' / 'manifest.json', encoding='utf-8').read()
        js = "((g, m) => { const miss = []; for (const k in m) if (m[k].games.includes(g)) {" \
             "if (!KIDS.voice.clips[k] || !KIDS.voice.clips[k].startsWith('data:audio/mpeg;base64,')) miss.push(k); }" \
             "return {n: Object.keys(KIDS.voice.clips).length, miss}; })('%s', %s)" % (GAME, mani)
