# -*- coding: utf-8 -*-
"""batch10 全量回归 driver：
①三款复验（verify_one_habit + verify_one_cs_si[chainsum+simon]）
②语音专项 verify_voice（chainsum/simon/habit）
③家长门+输入加关 E2E（core 级，habit 抽测）
...
run('verify:habit', 'python verify_one_habit.py', BASE)
run('verify:chainsum+simon', 'python verify_one_cs_si.py', BASE)
run('voice', 'python verify_voice.py chainsum simon habit', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，habit 抽测）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_habit', JSON.stringify({v:'1.0',game:'habit',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'habit', 'index.html').replace('\\', '/'))
