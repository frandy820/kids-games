# -*- coding: utf-8 -*-
"""batch10 两级入口验证：①batch10/index.html 三卡 ②主入口 6-7 岁区 batch10 三卡锚点
...
async def main():
    ...
    ok_all, miss = True, []
    n_links = 0
    for page in ('batch10/index.html', 'index.html'):
    ...
    async with async_playwright() as p:
        ...
        for name, rel in (('batch10/index.html', os.path.join(BASE, 'index.html')),
        ...
        await pg.wait_for_timeout(700)
        cards = await pg.evaluate("""(() => ['batch10/chainsum/index.html', 'batch10/simon/index.html', 'batch10/habit/index.html']
          .map(h => !!document.querySelector('a.card[href="' + h + '"]')))()""")
        rec('2 main-index batch10 3 cards', all(cards) and not errs, 'cards=%s' % cards)
        await pg.goto('file:///' + os.path.join(BASE, 'index.html').replace('\\', '/'))
        await pg.wait_for_timeout(700)
        cards2 = await pg.evaluate("""(() => ['chainsum/index.html', 'simon/index.html', 'habit/index.html']
          .map(h => !!document.querySelector('a.card[href="' + h + '"]')))()""")
        rec('1 batch10-index 3 cards', all(cards2) and not errs, 'cards=%s' % cards2)
        await ctx.close()
        await b.close()
