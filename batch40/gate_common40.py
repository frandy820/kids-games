# -*- coding: utf-8 -*-
"""batch40 首单元门禁（通用版）：verify selftest 复跑 + 真实页教学链/通关/写档 + clips 注入
用法: python gate_common39.py <game> <hook> [clips_n]
  game: ins|cbx|brk  hook: INS|CBX|BRK
  clips_n: manifest games 计数（ins=28/cbx=19/brk=16——ins 25+core 3（r26 注册 8 键后）/cbx 16+core 3/brk 13+core 3）"""
...
async def main():
    async with async_playwright() as p:
        ...
        demo_var = {'INS': '__insDemoR', 'CBX': '__cbxDemoR', 'BRK': '__brkDemoR'}[HOOK]
        demo_r = None
        for _ in range(60):
        ...
        for _ in range(30):
            ...
            if stars is not None:
                break
            await pg.wait_for_timeout(500)
        # 教学末步期望分款（SPEC-BATCH39 §4：crd='placed'/etm='picked'/cir='lit'
        #   ——tapX 枚举 SPEC §1-§3 各自定义，无 'right' 字面量，防 b37 复制笔误先例）
        demo_ok = demo_r == {'INS': 'picked', 'CBX': 'picked', 'BRK': 'done'}[HOOK]
        ...
        # G3 clips 注入（manifest games 全集+计数对账 ins 28（r26 后）/cbx 19/brk 16——审查 m-8 修）
        ctx = await b.new_context()
        pg = await ctx.new_page()
        await pg.goto(URL_R)
