/* ================= hidecup 藏猫猫摄像头 游戏数据（动物池 5 / 章配置 / 语音文案 / SVG）
   玩法（SPEC-BATCH34 §0.82/§1 + SPEC-R25 难度加深，5-6 岁视觉记忆）：经典杯子戏——
   当关主角动物探头亮相 1.5s → 躲进某杯 → 杯子换位（两杯位置互换动画，
   换位期杯恒盖住不露动物）→ 静止 800ms → 点杯开杯：
   动物在=right+动物蹦出+名音；空杯=wrong+「再想一想」（空开+盖回）。
   杯数 c/换位数 s（r25 章型表）：ch1=2/1、ch2=2/2（基线不动）、ch3=3/2（提速 900ms）、
   ch4 谱=qi0/2/4 hide c=4 s=4 + qi1/3 hidedual c=3 s=3（双动物双问+700ms）。
   hidedual（r25 新题型）：两只动物（关主 animA+关副 animB）各躲一杯，同串
   换位中各自追踪；第一步问 A 点对='half'（A 蹦出+链 [hc_right,hc_n_A,hc_n_B]
   链尾名音=第二问预告）→ 第二步问 B 点对=right/done（确认链尾=hc_n_B）。
   swaps=每次互换的两杯下标对 (a,b) a≠b，相邻两次换位至少一杯不同
   （禁连续同一对回滚式假换——c=2 时数值交替 (0,1)/(1,0) 唯一可行解）；
   answer=动物初始杯下标 start 经 swaps 序列逐次互换推导（hidedual 按 phase
   动态=当前步真值：phase0=answerA/phase1=answerB，双射恒推两答案互异）。
   点对=right+动物名音拼播（确认链 [hc_right, hc_n_<id>]——全 clip 无 keyless，
   契约 N 天然安全；r25 零新语音键，三维度全复用既有 11 条 hc_）。
   点错首错=错链 [hc_wrong, hc_hint]（豁免窗 5154=1656+150+3048+300 真时钟）+
   杯阵整体 wiggle（方向级不指杯）；miss≥2=正确杯 breathe（答案级）。 */
'use strict';

const INK = '#4A3B2E';                       // 统一暖棕描边（家族基调）

/* ---------- 动物池 5（SPEC §0.82：每关一主一只躲、其余不出场） ---------- */
...
/* ---------- 演出时序常量（SPEC §0.82 基线 + SPEC-R25 提速档） ----------
   r25 提速档（AUDIT-56 行99②）：dch1/dch2 恒 1100 基线不动 / dch3=900 /
   dch4=700（每档 -200ms，快换=追踪窗收窄）；教学恒 1600 慢速（教学链零改动）。 */
...
const SHOW_WIN_DUAL = 5400;                  // r25 双动物亮相链窗 ≥ 名音1440×2+150×2+hc_show1848+300=5328（两只轮流探头 3800 容纳）
const HALF_WIN = 5800;                       // r25 双动物第一步→第二步窗 ≥ hc_right2232+150+名音1440+150+名音1440+300=5712（链尾名音=第二问预告）
const SWAP_MS = 1100;                        // 每次换位动画 1100ms（dch1/dch2 基线不动）
const SWAP_MS_TUT = 1600;                    // 教学演示换位更慢（SPEC §1：2 杯 1 换慢速——教学链零改动）
const SWAP_MS_D3 = 900;                      // r25 dch3 提速档（中速）
const SWAP_MS_D4 = 700;                      // r25 dch4 提速档（快速）
const swapMsOf = dch => dch >= 4 ? SWAP_MS_D4 : (dch === 3 ? SWAP_MS_D3 : SWAP_MS);   // 档位单点（presentQuiz/doReplay 共用——重演同速不降难度）
...
/* ---------- 章配置（SPEC §0.82 真值表 + SPEC-R25 §R2 dch4 双型谱）
   dch4 主型 c=4 s=4（量域腿）+ dc/ds=3/3（双动物腿）；DUAL_KINDS=qi 位固定谱
   （r25：3 量域+2 双动物，两形态每关恒在场——确定性替代掷币，r24 同范式）。
...
const CH_CFG = { 1: { c: 2, s: 1 }, 2: { c: 2, s: 2 }, 3: { c: 3, s: 2 },
                 4: { c: 4, s: 4, dc: 3, ds: 3 } };
const DUAL_KINDS = ['hide', 'hidedual', 'hide', 'hidedual', 'hide'];
const CHAPTERS = {
  1: { name: '杯杯藏猫猫', hint: '杯子要换两次啦，跟紧看' },   // 预告 ch2 c=2 s=2
  2: { name: '多换一次',   hint: '三个杯杯来啦，仔细看' },     // 预告 ch3 c=3
  3: { name: '三杯换一换', hint: '四个杯杯来啦，藏两只' },     // 预告 ch4（r25：4杯+双动物）
  4: { name: '藏猫猫大挑战', hint: '新一轮藏猫猫开始' }        // 预告生成关
};
const GEN_HINTS = ['两个杯子换一次',        // dch1 c=2 s=1
                   '两个杯子换两次',        // dch2 c=2 s=2
                   '三个杯子换两次',        // dch3 c=3 s=2（r25 提速 900ms）
                   '四个杯子换四次'];       // dch4（r25：4杯4换+双动物+700ms）
...
/* ---------- 动物 SVG 工厂：animalSvg(id, size)——池 5 全定义（verify 单元①断言全在场）
   viewBox 0 0 120 120 卡通大头（5-6 岁一眼可辨）；根组 g[data-anim=<id>]。 ---------- */
