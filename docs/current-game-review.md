...
- L0：121/121；L1：121/121；**L2（verify PASS 证据）：121/121**；L3：G121 识字小课堂（2026-09-25 r2 后完成：真实通关+生字墙/阶梯反馈/音频失败取证）+ G034 藏猫猫摄像头（昨日 A-G 真实模式通关+结算弹层取证，今日未复验）；L4：0。
- 4 款慢 verify（G035 轮流浇花 12/12@131s / G072 机器人学跳舞 328/328@108s / G113 机器画师 399/399@138s / G116 井字棋小冠军 20/20@104s）：首轮探针 60-90s 超时窗内未出 title 被误标"挂起"，300s 复测全 PASS——纯探针超时问题，游戏无恙。
...
| G035 | 轮流浇花 | batch35 | ✓ | ? | — | 慢 verify（12/12（131s）），300s 复测通过 |
...
| G072 | 机器人学跳舞 | batch32 | ✓ | ? | — | 慢 verify（328/328（108s）），300s 复测通过 |
...
| G113 | 机器画师 | batch33 | ✓ | ? | — | 慢 verify（399/399（138s）），300s 复测通过 |
| G114 | 数据收集员 | batch34 | ✓ | verify PASS | — | 本轮自动化无阻断发现（深度未验） |
| G115 | 错题小医生 | batch35 | ✓ | verify PASS | — | 本轮自动化无阻断发现（深度未验） |
| G116 | 井字棋小冠军 | batch36 | ✓ | ? | — | 慢 verify（20/20（104s）），300s 复测通过 |
...
| G121 | 识字小课堂 | batch41 | ✓ | verify PASS | **完成（2026-09-25 r2 后）**：flat0 真实鼠标通关+生字墙点读取证+阶梯错误反馈（breathe/dimmed）+音频失败 toast，4 截图 shots-r2/ | 见 docs/literacy-game-design.md（诊断+设计+r2 五项实施） |
