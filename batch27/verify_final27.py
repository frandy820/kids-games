# -*- coding: utf-8 -*-
"""batch27 全量回归：三复验 + clips 注入 + 家长门抽测 + 两级入口 + href 全可达 + 真实路径推进"""
...
# R1-R3 三款独立复验（子进程串行）
for g in ['ruler', 'coin', 'notebird']:
    r = subprocess.run([sys.executable, os.path.join(BASE, 'verify_batch27.py'), g],
                       capture_output=True, text=True, encoding='utf-8', errors='replace')
    tail = (r.stdout or '').strip().split('\n')[-1] if r.stdout else ''
    chk('R%d %s 复验 11/11' % (RES.__len__() + 1, g), r.returncode == 0 and '11/11' in (r.stdout or ''), tail)

# R4 clips 注入（每款 ≥9 条 data:audio + 六键语音映射在场）
ok4, info4 = True, []
for g, pre in [('ruler', 'rul_'), ('coin', 'coi_'), ('notebird', 'not_')]:
...
try:
    with sync_playwright() as p:
        b = p.chromium.launch()
        pg = b.new_page()
        pg.goto('file:///' + os.path.join(BASE, 'notebird', 'index.html').replace(chr(92), '/'))
        pg.wait_for_timeout(3000)
        has_kids = pg.evaluate('() => typeof KIDS !== "undefined"')
        has_lock = pg.evaluate('() => document.body.innerHTML.includes("家长")')
        sv = pg.evaluate('() => { const s = localStorage.getItem("kidsgame_notebird"); return s ? JSON.parse(s) : null; }')
        pg.close(); b.close()
...
# R6 两级入口：主入口 81 卡 + batch27 入口 3 卡 + 三款子页存在
...
subs = [os.path.exists(os.path.join(BASE, g, 'index.html')) for g in ['ruler', 'coin', 'notebird']]
chk('R6 两级入口 81+3+子页', n_main == 81 and n_ent == 3 and all(subs),
...
# R8 真实路径推进（试玩 P1-1 验证缺口：verify 页 start(10) 直达测不到日历/多玩路径——
#   预置首日 6 关完成 → bonusSet(5)（家长面板同 API）→ autoSolve 走 flat6-10 →
#   断言 dch 推进 [2,2,2,2,3] + flat10=ch3 专属题型族 + 存档记录 2-1..3-0）
#   b27 试玩脚本曾漏设多玩数 → 引擎合法重放 flat5，误报「ch3 零出现+多玩不写档」）
from datetime import date
CH3_KINDS = {'ruler': ('cmp',), 'coin': ('sameval', 'combo'), 'notebird': ('higher',)}
HOOK = {'ruler': 'RU', 'coin': 'CO', 'notebird': 'NB'}
try:
    with sync_playwright() as p:
        b = p.chromium.launch()
        r8_infos = []
