# -*- coding: utf-8 -*-
"""batch27 首单元门禁（通用版）：verify selftest 复跑 + 真实页教学链/通关/写档 + clips 注入
用法: python gate_common27.py <game> <hook> [clips_n]
  game: ruler|coin|notebird  hook: RU|CO|NB"""
...
async def main():
    async with async_playwright() as p:
        ...
        await pg.goto(URL_V)
        title = ''
        for _ in range(360):          # r39：90×500ms=45s 罩不住 notebird（59 单元+唱窗+melody 驱动）——扩到 180s（主线 verify 同窗）
        ...
        demo_var = {'RU': '__ruDemoR', 'CO': '__coDemoR', 'NB': '__nbDemoR'}[HOOK]
        demo_r = None
        for _ in range(60):
            ...
            await pg.wait_for_timeout(500)
        tut = 'watch'
        for _ in range(40):                       # 等教学 handoff 完成（watch→help/solo；notebird ch1 顺序唱演示 ~10s）
        ...
        for _ in range(30):
            ...
            if stars is not None:
                break
            await pg.wait_for_timeout(500)
        # 演示返回值按款语义（SPEC-BATCH27）：三款演示完成一题均='right'（ruler 量对/coin 认对/notebird 听对）
