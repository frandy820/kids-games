# -*- coding: utf-8 -*-
"""batch15 全量回归 driver：
①三款复验（verify_one_cashier + verify_one_logicwho + verify_one_matchstick）
②语音专项 verify_voice（cashier/logicwho/matchstick）
③家长门+输入加关 E2E（core 级，cashier 抽测；模板=batch13/14 实证版）
...
run('verify:cashier', 'python verify_one_cashier.py', BASE)
run('verify:logicwho+matchstick', 'python verify_one_logicwho.py && python verify_one_matchstick.py', BASE)
run('voice', 'python verify_voice.py cashier logicwho matchstick', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，cashier 抽测；batch13/14 实证选择器）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_cashier', JSON.stringify({v:'1.0',game:'cashier',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'cashier', 'index.html').replace('\\', '/'))
...
print('')
print('BATCH15 REGRESSION %d/%d PASS' % (len(results) - len(bad), len(results)))
