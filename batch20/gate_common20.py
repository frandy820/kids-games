# -*- coding: utf-8 -*-
"""batch20 首单元门禁（通用版）：verify selftest 复跑 + 真实页教学链/通关/写档 + clips 注入
用法: python gate_common20.py <game> <hook> [clips_n]
  game: timecalc|memduel|quiz  hook: PF|AR|MM"""
...
async def main():
    async with async_playwright() as p:
        ...
        demo_var = {'PF': '__pfDemoR', 'AR': '__arDemoR', 'MM': '__mmDemoR', 'CD2': '__cd2DemoR', 'ST': '__stDemoR', 'BC': '__bcDemoR', 'CI': '__ciDemoR', 'GK': '__gkDemoR', 'SN': '__snDemoR', 'TC': '__tcDemoR', 'MD': '__mdDemoR', 'QZ': '__qzDemoR'}[HOOK]
        demo_r = None
        for _ in range(60):
        ...
        for _ in range(30):
            ...
            if stars is not None:
                break
            await pg.wait_for_timeout(500)
        rec('G2 教学链+通关+写档', demo_r == 'right' and r.get('done') and stars == 3 and not errs,
