# r36 主线收编（2026-09-21）：Executor 适配副本 sign/_src/verify_one_sign_r36.py 整替
# （URL/静态源读改回主线 batch26 口径）；适配详情 SPEC-R36-SIGN §R7。
# -*- coding: utf-8 -*-
...
URL = 'file:///' + os.path.join(HERE, 'sign', 'index.html').replace(chr(92), '/')
...
# ---- 源码级家族断言（Python 读文件）----
src = open(os.path.join(HERE, 'sign', '_src', 'game-main.js'), encoding='utf-8').read()
data = open(os.path.join(HERE, 'sign', '_src', 'game-data.js'), encoding='utf-8').read()
bld = open(os.path.join(HERE, 'sign', '_src', 'build.py'), encoding='utf-8').read()
