...
URL = 'file:///' + os.path.join(BASE, 'calendar', 'index.html').replace(chr(92), '/')
...
with sync_playwright() as p:
    ...
    for f in range(10, 20):
        ...
        for s in range(4):
            ...
            pg.wait_for_timeout(1000)   # r37 审查 minor4：cbound 确认窗余量仅 94ms，800 贴线防 clip 变长假红
...
src = open(os.path.join(BASE, 'calendar', '_src', 'game-main.js'), encoding='utf-8').read()
data = open(os.path.join(BASE, 'calendar', '_src', 'game-data.js'), encoding='utf-8').read()
bld = open(os.path.join(BASE, 'calendar', '_src', 'build.py'), encoding='utf-8').read()
