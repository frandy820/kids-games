# -*- coding: utf-8 -*-
"""6 岁人设试玩评估公共库（batch6/_obs6 · 2026-09-07）
...
BASE6 = os.path.dirname(os.path.abspath(__file__))
SHOT = os.path.join(BASE6, 'shots')
...
def save_result(game, data):
    p = os.path.join(BASE6, game + '_result.json')
    with open(p, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=1)
    print('[saved]', p)

def fmt_v(entries):
    """vlog 摘要行（queue 的数组参数拍平）"""
    out = []
    for e in entries:
        a = e.get('a', [])
        flat = []
        for x in a:
            if isinstance(x, list):
                flat.extend('%s|%s' % (i.get('key', ''), i.get('text', '')) for i in x if isinstance(i, dict))
            else:
                flat.append(str(x))
