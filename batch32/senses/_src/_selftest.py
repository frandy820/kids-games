# -*- coding: utf-8 -*-
"""senses verify 页复跑（独立 headless chromium.launch，禁 connect/禁杀浏览器）
+ 真实页冒烟（0 pageerror + 教学链 + autoSolve + 写档）+ flat0-19 全 autoSolve（multi 走真实提交链）。
r11 四族版：taps=判定步口径（单选 1 步/题，multi=提交 1 步）——每关恒 5。
用法: python batch32/senses/_src/_selftest.py"""
...
async def main():
    async with async_playwright() as p:
        ...
        await pg.goto(URL_V)
        title = ''
        for _ in range(240):                        # T46 带 10.1s 豁免窗等待，放宽到 120s
        ...
        try:
            ...
            for k in ('audit', 'tap', 'multi', 'anti', 'comp', 'frame', 'tutorial', 'swallow',
                      'clips', 'stars', 'contract', 'hints', 'estWin', 'modeled', 'speed'):
                if k in j.get('units', {}):
                    u = j['units'][k]
            ...
            for k, s in j.get('smokes', {}).items():
                if k == 'layout':
                    for one in s.get('sims', []):
                        print(' layout %s flat%-3s svgH=%s cards=%s hit=%s sub=%s scene=%s ox=%s pass=%s' %
                              (one['vp'], one['flat'], one['svgH'], one['cards'], one['hitOk'],
                               one.get('subOk'), one['sceneOk'], one['ox'], one['pass']))
                else:
        ...
        # 真实页冒烟（新 context=干净 localStorage，首日关+教学链）
        ...
        demo = None
        for _ in range(60):
            demo = await pg2.evaluate('window.__seDemoR || null')
            if demo:
                break
            await pg2.wait_for_timeout(500)
        tut = 'watch'
        for _ in range(40):
            tut = await pg2.evaluate('window.SE && SE.tutorial')
            if tut != 'watch':
                break
            await pg2.wait_for_timeout(500)
        r = await pg2.evaluate('SE.autoSolve()')
        lv = await pg2.evaluate('SE.currentLevel')
        stars = None
        for _ in range(30):                       # 写档在 winFlow celebrate 链异步落（≤15s）
            ...
            if stars is not None:
                break
            await pg2.wait_for_timeout(500)
        sv = await pg2.evaluate('JSON.parse(localStorage.getItem("kidsgame_senses") || "{}")')
        ...
        # flat0-19 全 autoSolve 通关（verify 页跑：无 winFlow/proceed 定时器竞态，UI 判定链全真）
        ...
        results = await pg3.evaluate('''(async () => {
          ...
          for (let f = 0; f < 20; f++) {
            SE.start(f);
            const r = await SE.autoSolve();
            out.push({ f: f, done: r.done, taps: r.taps,
                       dch: (SE.currentLevel && SE.currentLevel.dch) || null });
          }
          return out;
        })()''')
        bad = [x for x in results if not x['done'] or x['taps'] != 5]
        print('FLAT0-19 autoSolve: %d/20 done, bad=%s errs=%s dchSeq=%s' %
