...
②语音专项 verify_voice（countchick/spotdiff/connect）
③家长门+输入加关 E2E（core 级，countchick 抽测）
...
# ① 三款复验
for game in ('countchick', 'spotdiff', 'connect'):
    run('verify:%s' % game, 'python verify_one.py %s' % game, BASE)

# ② 语音专项
run('voice', 'python verify_voice.py countchick spotdiff connect', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，countchick 抽测）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_countchick', JSON.stringify({v:'1.0',game:'countchick',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'countchick', 'index.html').replace('\\', '/'))
