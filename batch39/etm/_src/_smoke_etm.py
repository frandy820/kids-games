# -*- coding: utf-8 -*-
"""etm 冒烟：verify selftest（等 title）+ 真实页教学链/首关（watch→turn→autoSolve）。
无头独立 launch（--mute-audio），单 page 串行；禁 connect/禁杀浏览器。"""
import asyncio, io, sys
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
from playwright.async_api import async_playwright

URL = 'file:///F:/claudecode/projects/active/kids-games/batch39/etm/index.html'
...
async def main():
    async with async_playwright() as p:
        ...
        if title and 'PASS' not in title:
            units = await pg.evaluate('window.__etmVlog ? window.__etmVlog.units : {}')
        ...
        demo_r = None
        for _ in range(60):
            demo_r = await pg.evaluate('window.__etmDemoR || null')
            if demo_r:
                break
            await pg.wait_for_timeout(500)
        tut = 'watch'
        for _ in range(40):
            tut = await pg.evaluate('window.ETM && ETM.tutorial')
            if tut != 'watch':
                break
            await pg.wait_for_timeout(500)
        r = await pg.evaluate('ETM.autoSolve()')
        stars = None
        for _ in range(30):
        ...
        lv = await pg.evaluate('ETM.currentLevel ? JSON.stringify(ETM.currentLevel) : null')
