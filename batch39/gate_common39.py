# -*- coding: utf-8 -*-
"""batch39 首单元门禁（通用版）：verify selftest 复跑 + 真实页教学链/通关/写档 + clips 注入
用法: python gate_common39.py <game> <hook> [clips_n]
  game: crd|etm|cir  hook: CRD|ETM|CIR
  clips_n: manifest games 计数（crd/cir=8 款语音 5+core 3；etm=48——r49 五档+mix 扩容后，
  段二已实参传入，docstring 勘正 r49-fix m3）"""
...
async def main():
    async with async_playwright() as p:
        ...
        demo_var = {'CRD': '__crdDemoR', 'ETM': '__etmDemoR', 'CIR': '__cirDemoR'}[HOOK]
        demo_r = None
        for _ in range(60):
        ...
        demo_ok = demo_r == {'CRD': 'placed', 'ETM': 'picked', 'CIR': 'lit'}[HOOK]
        ...
        # G3 clips 注入（manifest games 全集+计数对账 三款均 8=款 5+core 3）
        ctx = await b.new_context()
        pg = await ctx.new_page()
        await pg.goto(URL_R)
