# -*- coding: utf-8 -*-
"""batch14 两级入口验证：①batch14/index.html 三卡 ②主入口 5-6 岁区 batch14 三卡锚点
...
async def main():
    ...
    ok_all, miss = True, []
    n_links = 0
    for page in ('batch14/index.html', 'index.html'):
    ...
    async with async_playwright() as p:
        ...
        for name, rel in (('batch14/index.html', os.path.join(BASE, 'index.html')),
        ...
        await pg.wait_for_timeout(700)
        cards = await pg.evaluate("""(() => ['batch14/blocks/index.html', 'batch14/multibattle/index.html', 'batch14/numberdet/index.html']
          .map(h => !!document.querySelector('a.card[href="' + h + '"]')))()""")
        rec('2 main-index batch14 3 cards', all(cards) and not errs, 'cards=%s' % cards)
        await pg.goto('file:///' + os.path.join(BASE, 'index.html').replace('\\', '/'))
        await pg.wait_for_timeout(700)
        cards2 = await pg.evaluate("""(() => ['blocks/index.html', 'multibattle/index.html', 'numberdet/index.html']
          .map(h => !!document.querySelector('a.card[href="' + h + '"]')))()""")
        rec('1 batch14-index 3 cards', all(cards2) and not errs, 'cards=%s' % cards2)
        await ctx.close()
        await b.close()
