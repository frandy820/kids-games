# -*- coding: utf-8 -*-
"""batch28 全量回归：三复验 + clips 注入 + 家长门抽测 + 两级入口 + href 全可达 + 真实路径推进"""
...
# R1-R3 三款独立复验（子进程串行）
for g in ['coder', 'shapecount', 'conserve']:
    r = subprocess.run([sys.executable, os.path.join(BASE, 'verify_batch28.py'), g],
                       capture_output=True, text=True, encoding='utf-8', errors='replace')
    tail = (r.stdout or '').strip().split('\n')[-1] if r.stdout else ''
    chk('R%d %s 复验全绿' % (RES.__len__() + 1, g), r.returncode == 0 and 'PASS' in (r.stdout or ''), tail)

# R4 clips 注入（每款 ≥9 条 data:audio + 六键语音映射在场；shapecount 前缀=shc_，sha_ 撞车事故后改）
ok4, info4 = True, []
for g, pre in [('coder', 'cod_'), ('shapecount', 'shc_'), ('conserve', 'cnv_')]:
    ...
    if n_audio < 2 or missing:   # 前缀拼接写法：base64 前缀字符串出现次数少，键存在性为准
        ok4 = False; info4.append('%s miss=%s' % (g, missing))
chk('R4 三款 clips 六键+注入在场', ok4, info4 or 'ok')

# R5 家长门抽测（notebird 真实页：KIDS 在场+家长入口+存档会话字段）
from playwright.sync_api import sync_playwright
try:
    with sync_playwright() as p:
        b = p.chromium.launch()
        pg = b.new_page()
        pg.goto('file:///' + os.path.join(BASE, 'conserve', 'index.html').replace(chr(92), '/'))
        pg.wait_for_timeout(3000)
        has_kids = pg.evaluate('() => typeof KIDS !== "undefined"')
        has_lock = pg.evaluate('() => document.body.innerHTML.includes("家长")')
        sv = pg.evaluate('() => { const s = localStorage.getItem("kidsgame_conserve"); return s ? JSON.parse(s) : null; }')
        pg.close(); b.close()
...
# R6 两级入口：主入口 84 卡 + batch28 入口 3 卡 + 三款子页存在
...
subs = [os.path.exists(os.path.join(BASE, g, 'index.html')) for g in ['coder', 'shapecount', 'conserve']]
chk('R6 两级入口 84+3+子页', n_main == 84 and n_ent == 3 and all(subs),
    'main=%d ent=%d subs=%s' % (n_main, n_ent, subs))

# R7 href 全可达（主入口+batch27 入口所有相对链接目标存在——b24 坑①：卡数对账≠链接可达）
bad = []
...
# R8 真实路径推进（b27 试玩 P1-1 验证缺口定版常驻：verify 页 start(10) 直达测不到日历/多玩路径——
#   预置首日 6 关完成 → bonusSet(5)（家长面板同 API）→ autoSolve 走 flat6-10 →
#   断言 dch 推进 [2,2,2,2,3] + flat10=ch3 专属题型族 + 存档记录 2-1..3-0）
#   b27 试玩脚本曾漏设多玩数 → 引擎合法重放 flat5，误报；b28 起每批必带）
from datetime import date
CH3_KINDS = {'coder': ('run', 'path'), 'shapecount': ('more',), 'conserve': ('same', 'add')}
HOOK = {'coder': 'CD', 'shapecount': 'SC', 'conserve': 'CV'}
try:
    with sync_playwright() as p:
        b = p.chromium.launch()
        r8_infos = []
        ...
        for g in ['coder', 'shapecount', 'conserve']:
            ...
            for f in range(6, 11):
                ...
                dchs.append(st['dch'])
                if f == 10:
                    kinds10 = pg.evaluate('() => genLevel(10).quizzes.map(q => q.kind)')   # currentLevel getter 不含 quizzes；startLevel 同源纯函数
                pg.evaluate('() => %s.autoSolve()' % HOOK[g])
            ...
            if dchs:
                ...
                ok_g = (dchs == [2, 2, 2, 2, 3] and
                        all(k in CH3_KINDS[g] for k in kinds10) and len(kinds10) == 5 and
                        len(rec_keys) == 5)
