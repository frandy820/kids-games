...
用法: python gate_common28.py <game> <hook> [clips_n]
  game: coder|shapecount|conserve  hook: CD|SC|CV"""
...
async def main():
    async with async_playwright() as p:
        ...
        demo_var = {'CD': '__cdDemoR', 'SC': '__scDemoR', 'CV': '__cvDemoR'}[HOOK]
        demo_r = None
        for _ in range(60):
        ...
        for _ in range(30):
            ...
            if stars is not None:
                break
            await pg.wait_for_timeout(500)
        # 演示返回值按款语义（SPEC-BATCH28）：三款演示完成一题均='right'（coder 走到/shapecount 数对/conserve 比对）
