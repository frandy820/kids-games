...
语音 clip 已全部合成在场（coder2 10 条=cd2_ 中文 7+core 共享 3，
由主会话注入 manifest games:['coder2']）：
注入失败必须 sys.exit(3)，禁降级空串构建"""
import pathlib, sys

ROOT = pathlib.Path(__file__).resolve().parent          # _src/
OUT = ROOT.parent / 'index.html'                        # batch18/coder2/index.html
...
# 语音 clips 注入（coder2 16 条已合成在场；失败=构建失败退出码 3，不降级）
try:
    sys.path.insert(0, r'F:/claudecode/projects/active/kids-games/voice')
    from inject_clips import clips_js
    clips = clips_js('coder2')
except SystemExit:
    raise SystemExit(3)
except Exception as e:
    print('FATAL: coder2 clips 注入失败:', e)
    sys.exit(3)
