# -*- coding: utf-8 -*-
"""batch4 全量回归 driver（clips 注入后产物）：
①clock/times verify_one（verify=1/真实通关/双viewport/离线/截图/钩子）
②sudoku 专项 verify_sudoku + clock 拖针专项 verify_dial
③语音专项 verify_voice（clock/times/sudoku）
④家长门+输入加关 E2E（core 级，times 抽测）
...
# ① 点选款复验（clips 注入后）
for game, hook, btn in (('clock', 'CLK', '.opt'), ('times', 'TIM', '.fish-btn')):
    run('reverify:%s' % game, 'python verify_one.py %s %s "%s" items' % (game, hook, btn), BASE)

# ② 专项：sudoku（board/fill 形态）+ clock 章 3 拖针
run('sudoku', 'python verify_sudoku.py', BASE)
run('clock-dial', 'python verify_dial.py', BASE)

# ③ 语音专项
run('voice', 'python verify_voice.py clock times sudoku', os.path.join(BASE, '..', 'batch1'))

# ④ 家长门+加关 E2E（core 级，times 抽测）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_times', JSON.stringify({v:'1.0',game:'times',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'times', 'index.html').replace('\\', '/'))
