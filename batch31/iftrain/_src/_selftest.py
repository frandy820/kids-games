# -*- coding: utf-8 -*-
"""iftrain verify 页复跑（独立 headless chromium.launch，禁 connect/禁杀浏览器）
+ 真实页冒烟（0 pageerror + 教学链 + autoSolve + 写档）+ flat0-19 全 autoSolve。
用法: python batch31/iftrain/_src/_selftest.py"""
...
async def main():
    async with async_playwright() as p:
        ...
        try:
            ...
            for k in ('audit', 'tap', 'tapRuleback', 'frame', 'tutorial', 'swallow', 'clips',
        ...
        demo = None
        for _ in range(60):
            demo = await pg2.evaluate('window.__ifDemoR || null')
            if demo:
                break
            await pg2.wait_for_timeout(500)
        tut = 'watch'
        for _ in range(40):
            tut = await pg2.evaluate('window.IF && IF.tutorial')
            if tut != 'watch':
                break
            await pg2.wait_for_timeout(500)
        r = await pg2.evaluate('IF.autoSolve()')
        lv = await pg2.evaluate('IF.currentLevel')
        stars = None
        for _ in range(30):                       # 写档在 winFlow celebrate 链异步落（≤15s）
            ...
            if stars is not None:
                break
            await pg2.wait_for_timeout(500)
        sv = await pg2.evaluate('JSON.parse(localStorage.getItem("kidsgame_iftrain") || "{}")')
        ...
        results = await pg3.evaluate('''(async () => {
          ...
          for (let f = 0; f < 20; f++) {
            IF.start(f);
            const r = await IF.autoSolve();
            out.push({ f: f, done: r.done, taps: r.taps,
                       dch: (IF.currentLevel && IF.currentLevel.dch) || null });
          }
          return out;
        })()''')
