# -*- coding: utf-8 -*-
"""batch38 首单元门禁（通用版）：verify selftest 复跑 + 真实页教学链/通关/写档 + clips 注入
用法: python gate_common38.py <game> <hook> [clips_n]
  game: stamp|libr|gear  hook: ST|LB|GR
  clips_n: manifest games 计数（libr=8 款 5+core 3；r 批升档后实数：stamp v3=12/
    gear v3=10——以传参为准，缺省跳过计数对账）"""
...
async def main():
    async with async_playwright() as p:
        ...
        demo_var = {'ST': '__stDemoR', 'LB': '__lbDemoR', 'GR': '__grDemoR'}[HOOK]
        demo_r = None
        for _ in range(60):
        ...
        for _ in range(30):
            ...
            if stars is not None:
                break
            await pg.wait_for_timeout(500)
        # 教学末步期望分款（SPEC-BATCH38 §4：stamp='stamped'/libr='shelved'/gear='meshed'
        #   ——tapX 枚举 SPEC §1-§3 各自定义，无 'right' 字面量，防 b37 复制笔误先例）
        demo_ok = demo_r == {'ST': 'stamped', 'LB': 'shelved', 'GR': 'meshed'}[HOOK]
