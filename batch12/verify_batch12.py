# -*- coding: utf-8 -*-
"""batch12 全量回归 driver：
①三款复验（verify_one_fraction + verify_one_shp_so[shapeshome+sortsize]）
②语音专项 verify_voice（shadow/shapeshome/sortsize）
③家长门+输入加关 E2E（core 级，divide 抽测）
...
def run(name, cmd, cwd):
    # r15：子脚本 stdout 均强制 UTF-8，driver 侧显式 utf-8 解码（默认 GBK 撞中文崩溃丢 FAIL 详情）
    r = subprocess.run(cmd, cwd=cwd, capture_output=True, text=True, encoding='utf-8',
                       errors='replace', shell=True, timeout=1200)
...
run('verify:fraction', 'python verify_one_fraction.py', BASE)
run('verify:div+clm', 'python verify_one_div_clm.py', BASE)   # 2026-09-16 r15 拆分后=divide 专属
run('verify:column', 'python verify_one_column.py', BASE)     # r15 独立（8 题/关+标记操作步+两步/三位数）
run('voice', 'python verify_voice.py divide fraction column', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，divide 抽测）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_divide', JSON.stringify({v:'1.0',game:'divide',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'divide', 'index.html').replace('\\', '/'))
