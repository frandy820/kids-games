...
HERE = os.path.dirname(os.path.abspath(__file__))          # _src/（适配版归档位）
URL = 'file:///' + os.path.join(os.path.dirname(HERE), 'index.html').replace(chr(92), '/') + '?verify=1'
...
src = io.open(os.path.join(HERE, 'game-main.js'), encoding='utf-8').read()
s10 = []
...
data_src = io.open(os.path.join(HERE, 'game-data.js'), encoding='utf-8').read()
s11 = []
