# -*- coding: utf-8 -*-
"""batch25 全量回归 driver：
①三款独立复验（verify_one_story3 + verify_one_emo + verify_one_habitat）
②语音专项 verify_voice（story3/emo/habitat）
③家长门+输入加关 E2E（core 级，habitat 抽测）
④两级入口（batch25/index.html 三卡+主入口 75 卡+href 全可达扫描——b24 契约+三产物直开冒烟）
...
run('verify:story3', 'python verify_one_story3.py', BASE, timeout=3000)
run('verify:emo', 'python verify_one_emo.py', BASE, timeout=3000)
run('verify:habitat', 'python verify_one_habitat.py', BASE, timeout=3000)
run('voice', 'python verify_voice.py story3 emo habitat', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，habitat 抽测；算术题→input 填 5→确定→limit+5）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_habitat', JSON.stringify({v:'1.0',game:'habitat',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'habitat', 'index.html').replace('\\', '/'))
...
# ④ 两级入口+href 全可达（b24 契约：卡数对账≠链接可达）
def entries():
    with sync_playwright() as p:
        b = p.chromium.launch()
        pg = b.new_page()
        ...
        ok1 = sum(1 for c in cards if '故事排序' in c) >= 1 and sum(1 for c in cards if '情绪脸谱' in c) >= 1 and sum(1 for c in cards if '动物家园' in c) >= 1
        main_pg = b.new_page()
        main_pg.goto('file:///' + os.path.join(BASE, '..', 'index.html').replace('\\', '/'))
        main_pg.wait_for_timeout(800)
        main_cards = main_pg.evaluate("() => Array.from(document.querySelectorAll('.card')).map(c => c.textContent.trim())")
        ok2 = sum(1 for c in main_cards if '故事排序' in c) >= 1 and sum(1 for c in main_cards if '情绪脸谱' in c) >= 1 and sum(1 for c in main_cards if '动物家园' in c) >= 1 and len(main_cards) == 75
        # href 全可达扫描（主入口全部卡片）
        hrefs = main_pg.evaluate("() => Array.from(document.querySelectorAll('a.card')).map(a => a.getAttribute('href'))")
        missing = [h for h in hrefs if not os.path.exists(os.path.normpath(os.path.join(BASE, '..', h)))]
        ok_href = not missing and len(hrefs) == 75
        # 三款产物页直开冒烟
        n = 0
        for g in ('story3', 'emo', 'habitat'):
            pg3 = b.new_page()
        ...
        b.close()
        assert ok1 and ok2 and ok_href and ok3, '入口断言 b25=%s main=%s href=%s(%s) smoke=%s' % (ok1, ok2, ok_href, missing[:3], ok3)
    print('[PASS] entries 两级入口+href 全可达（主入口 75 卡）+三产物冒烟')
