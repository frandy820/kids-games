# -*- coding: utf-8 -*-
"""batch21 全量回归 driver：
①三款独立复验（verify_one_feed + verify_one_bubble + verify_one_bridge）
②语音专项 verify_voice（feed/bubble/bridge）
③家长门+输入加关 E2E（core 级，quiz 抽测；模板=batch15-19 实证版）
④两级入口（batch20/index.html 三卡+主入口 ≥120 卡含三卡）
...
run('verify:feed', 'python verify_one_feed.py', BASE, timeout=3000)
run('verify:bubble', 'python verify_one_bubble.py', BASE, timeout=3000)
run('verify:bridge', 'python verify_one_bridge.py', BASE, timeout=3000)
run('voice', 'python verify_voice.py feed bubble bridge', os.path.join(BASE, '..', 'batch1'))
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_feed', JSON.stringify({v:'1.0',game:'feed',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'feed', 'index.html').replace('\\', '/'))
...
def entries():
    with sync_playwright() as p:
        ...
        ok1 = sum(1 for c in cards if '喂小兔' in c) >= 1 and sum(1 for c in cards if '泡泡数数' in c) >= 1 and sum(1 for c in cards if '过河石桥' in c) >= 1
        pg2 = b.new_page()
        ...
        # 主入口卡数（120 款扩容后定稿 120，2026-09-14）——r8 审查 m2：下限钉 120 防缩水（跌卡即红）
        ok2 = sum(1 for c in main_cards if '喂小兔' in c) >= 1 and sum(1 for c in main_cards if '泡泡数数' in c) >= 1 and sum(1 for c in main_cards if '过河石桥' in c) >= 1 and len(main_cards) >= 120
        # 三款产物页直开冒烟
        n = 0
        for g in ('feed', 'bubble', 'bridge'):
            pg3 = b.new_page()
