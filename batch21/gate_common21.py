# -*- coding: utf-8 -*-
"""batch21 首单元门禁（通用版）：verify selftest 复跑 + 真实页教学链/通关/写档 + clips 注入
用法: python gate_common21.py <game> <hook> [clips_n]
  game: feed|bubble|bridge  hook: FD|BB|BG"""
...
async def main():
    async with async_playwright() as p:
        ...
        demo_var = {'PF': '__pfDemoR', 'AR': '__arDemoR', 'MM': '__mmDemoR', 'CD2': '__cd2DemoR', 'ST': '__stDemoR', 'BC': '__bcDemoR', 'CI': '__ciDemoR', 'GK': '__gkDemoR', 'SN': '__snDemoR', 'TC': '__tcDemoR', 'MD': '__mdDemoR', 'QZ': '__qzDemoR', 'FD': '__fdDemoR', 'BB': '__bbDemoR', 'BG': '__bgDemoR'}[HOOK]
        demo_r = None
        for _ in range(60):
        ...
        for _ in range(30):
            ...
            if stars is not None:
                break
            await pg.wait_for_timeout(500)
        # 演示返回值按款语义：选择/计数款=答对一题 'right'；bridge r9 演示找对错石='found'
        # （r9 改造前旧玩法='step' 走 2 步——玩法已换，判对证据随之更新）
        demo_ok = demo_r == 'right' or (GAME == 'bridge' and demo_r == 'found')
