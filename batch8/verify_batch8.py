# -*- coding: utf-8 -*-
"""batch8 全量回归 driver：
①三款复验（verify_one_picto + verify_one_nb_wen[neighbors+worden]）
②语音专项 verify_voice（neighbors/picto/worden）
③家长门+输入加关 E2E（core 级，worden 抽测）
...
# ① 三款复验
run('verify:picto', 'python verify_one_picto.py', BASE)
run('verify:neighbors+worden', 'python verify_one_nb_wen.py', BASE)

# ② 语音专项
run('voice', 'python verify_voice.py neighbors picto worden', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，worden 抽测）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_worden', JSON.stringify({v:'1.0',game:'worden',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'worden', 'index.html').replace('\\', '/'))
