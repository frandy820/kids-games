# -*- coding: utf-8 -*-
"""batch13 全量回归 driver：
①三款复验（verify_one_wordprob + verify_one_money + verify_one_grid）
②语音专项 verify_voice（money/wordprob/grid）
③家长门+输入加关 E2E（core 级，wordprob 抽测）
...
def run(name, cmd, cwd):
    # r15：子脚本 stdout 均强制 UTF-8，driver 侧显式 utf-8 解码（默认 GBK 撞中文崩溃丢 FAIL 详情——照 batch12 同坑修法）
    r = subprocess.run(cmd, cwd=cwd, capture_output=True, text=True, encoding='utf-8',
                       errors='replace', shell=True, timeout=1800)
...
run('verify:wordprob', 'python verify_one_wordprob.py', BASE)
run('verify:money+grid', 'python verify_one_money.py && python verify_one_grid.py', BASE)
run('voice', 'python verify_voice.py money wordprob grid', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，wordprob 抽测）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_wordprob', JSON.stringify({v:'1.0',game:'wordprob',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'wordprob', 'index.html').replace('\\', '/'))
