# -*- coding: utf-8 -*-
"""batch9 全量回归 driver：
①三款复验（verify_one_wis + verify_one_mir_mem[mirror+memgrid]）
②语音专项 verify_voice（whereistand/mirror/memgrid）
③家长门+输入加关 E2E（core 级，memgrid 抽测）
...
def run(name, cmd, cwd):
    r = subprocess.run(cmd, cwd=cwd, capture_output=True, text=True, shell=True, timeout=900)
...
# ① 三款复验
run('verify:whereistand', 'python verify_one_wis.py', BASE)
run('verify:mirror+memgrid', 'python verify_one_mir_mem.py', BASE)

# ② 语音专项
run('voice', 'python verify_voice.py whereistand mirror memgrid', os.path.join(BASE, '..', 'batch1'))

# ③ 家长门+加关 E2E（core 级，memgrid 抽测）
...
def parent_gate():
    with sync_playwright() as p:
        ...
        pg.add_init_script("localStorage.setItem('kidsgame_memgrid', JSON.stringify({v:'1.0',game:'memgrid',firstDay:'%s',lastDay:'%s',levels:{},dailyMin:{},settings:{sound:false,tts:false,vol:0.3},restTip:{day:'',shown:0}}));" % (today, today))
        pg.goto('file:///' + os.path.join(BASE, 'memgrid', 'index.html').replace('\\', '/'))
