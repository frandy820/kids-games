# -*- coding: utf-8 -*-
"""tangram 单文件拼接：head + body + core.js(原样) + clips + 游戏代码 → ../index.html"""
import pathlib, sys
...
eng = (ROOT / 'game-core.js').read_text(encoding='utf-8')
main = (ROOT / 'game-main.js').read_text(encoding='utf-8')
# 语音 clips 注入（管线未就绪时降级为空串，主会话补管线后重建即可）
try:
    sys.path.insert(0, 'F:/claudecode/projects/active/kids-games/voice')
    from inject_clips import clips_js
    clips = clips_js('tangram')
except ImportError:
    clips = ''

# 硬性检查 1：JS 内不得出现字面 </script>（会提前闭合标签）
for name, s in [('core', core), ('data', data), ('eng', eng), ('main', main), ('clips', clips)]:
